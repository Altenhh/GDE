...
namespace GDE.Tests.Visual.TestSceneEditor
{
    public class TestSceneIDMigration : TestScene
    {
        ...
        {
            EditorScreen screen = new EditorScreen(0, new Level());
            Editor editor = screen.Editor;
            IDMigrationPanel panel = new IDMigrationPanel(screen);
            ...
            AddAssert("Match current selection", () => panel.CurrentStepList.SelectedStepIndices.SetEquals(new int[] { 0, 1 }));
            AddAssert("Check common selected values", () => panel.CommonIDMigrationStep.Value.HasValues(1, 5, 15, 19));
            AddStep("Append step at index 2 to selection", () => panel.CurrentStepList.SelectStep(2, true));
            AddAssert("Match current selection", () => panel.CurrentStepList.SelectedStepIndices.SetEquals(new int[] { 0, 1, 2 }));
            ...
            AddAssert("Match current selection", () => panel.CurrentStepList.SelectedStepIndices.SetEquals(new int[] { }));
            ...
            AddRepeatStep("Create 4 new identical steps", () => panel.CurrentStepList.AddStep(new SourceTargetRange(1, 5, 15)), 4);
            ...
            AddAssert("Check common selected values", () => panel.CommonIDMigrationStep.Value.HasValues(1, 5, 15, 19));
            AddAssert("Match current selection", () => panel.CurrentStepList.SelectedStepIndices.SetEquals(new int[] { 0, 1, 2, 3 }));
